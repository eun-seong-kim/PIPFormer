...
class Context_Attention(nn.Module):
    def __init__(self, dim, heads = 8, dim_head = 64):
        ...
        self.external_k = nn.Parameter(torch.randn([1, heads, 1, dim_head]))
        self.external_v = nn.Parameter(torch.randn([1, heads, 1, dim_head]))
        
    def forward(self, x, mask, context):
        B, _, H, W = x.shape
        
        qkv = self.to_qkv(x) # B, 3C, H, W
        qkv = qkv.reshape(B, self.heads, -1, H*W).permute(0, 1, 3, 2) # B, head, 3c, n -> B, head, n, 3c
        q, k, v = qkv.chunk(3, dim = -1) # B, head, n, c
        k = torch.concat([self.external_k.tile(B, 1, 1, 1), k], 2)
        v = torch.concat([self.external_v.tile(B, 1, 1, 1), v], 2)
        
        with torch.no_grad():
            mask = F.interpolate(mask, (H, W), mode='bilinear', align_corners=True, antialias=False)
            mask = mask.reshape(B, 1, H*W, 1)
            context = F.interpolate(context, (H, W), mode='bilinear', align_corners=True, antialias=False)
            context = context.reshape(B, 1, 1, H*W)
            add = torch.ones_like(context[:, :, :, :1]) if torch.sum(context) == 0 else torch.zeros_like(context[:, :, :, :1])
            context = torch.cat([add, context], -1)
            context_mask = mask * context

        dots = (torch.matmul(q, k.transpose(-1, -2)) + self.context_scale * context_mask) * self.scale
        attn = self.attend(dots)
        out = torch.matmul(attn, v) # B, head, n, c
        out = out.permute(0, 1, 3, 2).reshape(B, -1, H, W) # B, head, c, n -> B, C, H, W
